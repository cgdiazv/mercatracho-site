...
export default function Weather({ cityQuery, displayName }: WeatherProps) {
  ...
  if (loading || !data) {
    return (
      <div className="flex items-center gap-2 rounded-full border border-[#ebf0f6] bg-white px-3 py-1.5 animate-pulse">
        <div className="h-3 w-6 rounded bg-gray-100"></div>
      </div>
    );
  }
  ...
  return (
    <Link
      href={`/clima/${encodeURIComponent(cityQuery)}`}
      className="flex items-center gap-2 rounded-full border border-[#ebf0f6] bg-white px-3 py-1.5 shadow-sm transition hover:border-[#2175eb] hover:shadow-md"
    >
